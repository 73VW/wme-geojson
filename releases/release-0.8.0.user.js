...
var panel$1 = {
	...
	distanceFilter: {
		...
		clear: "Clear list",
		viewsHeading: "Views ({{count}})",
		viewButton: "View {{index}} — z{{zoom}}",
...
var panel = {
	...
	distanceFilter: {
		...
		viewsHeading: "Vues ({{count}})",
		viewButton: "Vue {{index}} — z{{zoom}}",
		viewCenter: "lon={{lon}}&lat={{lat}}"
	},
	progress: {
		empty: "Non démarré",
...
function computeBboxViews(args) {
    const { labels, viewportLonSpanAtCurrentZoom, viewportLatSpanAtCurrentZoom, currentZoom, minZoom, maxZoom, } = args;
    if (labels.length === 0)
        return [];
    const lonSpanAtZoom = (z) => viewportLonSpanAtCurrentZoom * Math.pow(2, currentZoom - z);
    const latSpanAtZoom = (z) => viewportLatSpanAtCurrentZoom * Math.pow(2, currentZoom - z);
    const fitsAtZoom = (bLon, bLat, z) => lonSpanAtZoom(z) >= bLon && latSpanAtZoom(z) >= bLat;
    const bestZoom = (bLon, bLat) => {
        if (bLon === 0 && bLat === 0) {
            return maxZoom;
        }
        const ratio = Math.max(bLon / viewportLonSpanAtCurrentZoom, bLat / viewportLatSpanAtCurrentZoom);
...
class MatchPanel {
    ...
    buildDistanceFilter() {
        ...
        status.textContent = instance.t("panel.distanceFilter.matched", {
            visible: 0,
            requested: 0,
        });
        ...
        const updateStatus = (visible, requested) => {
            status.textContent = instance.t("panel.distanceFilter.matched", {
                visible,
                requested,
            });
    ...
    refreshBboxList() {
        if (!this.bboxListEl || !this.bboxHeadingEl)
            return;
        ...
        const [minLon, minLat, maxLon, maxLat] = rawExtent;
        ...
        this.bboxHeadingEl.textContent = instance.t("panel.distanceFilter.viewsHeading", {
            count: views.length,
        });
