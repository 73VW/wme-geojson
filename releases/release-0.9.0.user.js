...
var panel$1 = {
	...
	range: {
		title: "Visible distance range",
		window: "{{min}} km – {{max}} km"
	},
	distanceFilter: {
		title: "Filter labels by distance list",
		help: "Show only labels matching the listed kilometres (rounded to 100 m). Separate by comma, space or newline.",
		placeholder: "e.g. 0.5, 1.2, 3.4",
		matched: "{{visible}} visible / {{requested}} requested",
		clear: "Clear list",
		runProcess: "Compute views",
		stopProcess: "Stop",
		processing: "Computing view {{done}}/{{total}}…",
		stale: "List out of date — re-run",
		viewButton: "View for {{distance}} km",
		viewButtonIndexed: "View {{index}} for {{distance}} km",
		viewCenter: "lon={{lon}}&lat={{lat}}"
	},
	progress: {
		empty: "Not started",
...
var errors = {
	fetchFailed: "Impossible de charger le tracé GeoJSON depuis l'URL.",
	invalidGeometry: "Le GeoJSON a un type de géométrie invalide ou non supporté."
};
var panel = {
	title: "WME GeoJSON",
	trackInfo: {
		id: "ID du tracé : {{id}}",
		length: "Longueur : {{km}} km"
	},
	status: {
		idle: "En attente",
		walking: "Parcours en cours…",
		done: "Terminé",
		cancelled: "Annulé",
		error: "Erreur"
	},
	buttons: {
		centerOnTrack: "Centrer sur le tracé",
		start: "Lancer la correspondance",
		stop: "Arrêter",
		selectAll: "Tout sélectionner",
		selectAllConfirm: "Tout sélectionner",
		cancel: "Annuler"
	},
	modal: {
		largeSelectionWarning: "Vous êtes sur le point de sélectionner {{count}} segments. Les grandes sélections peuvent être lentes ou échouer. Continuer ?"
	},
	errors: {
		selectionFailed: "Échec de la sélection : {{error}}"
	},
	range: {
		title: "Plage de distance visible",
		window: "{{min}} km – {{max}} km"
	},
	distanceFilter: {
		title: "Filtrer les labels par liste de distances",
		help: "N'afficher que les labels correspondant aux kilomètres listés (arrondi aux 100 m). Séparer par virgule, espace ou retour à la ligne.",
		placeholder: "ex. 0.5, 1.2, 3.4",
		matched: "{{visible}} visibles / {{requested}} demandés",
		clear: "Vider la liste",
		runProcess: "Calculer les vues",
		stopProcess: "Arrêter",
		processing: "Calcul de la vue {{done}}/{{total}}…",
		stale: "Liste périmée — relancer",
		viewButton: "Vue pour {{distance}} km",
		viewButtonIndexed: "Vue {{index}} pour {{distance}} km",
		viewCenter: "lon={{lon}}&lat={{lat}}"
	},
	progress: {
		empty: "Non démarré",
		running: "{{visited}} / {{total}} cellules"
	},
	results: {
		empty: "Aucune correspondance.",
		count: "{{count}} segment(s) trouvé(s)",
		item: "Segment {{id}}",
		unavailable: "Segment indisponible",
		active: "(actif)"
	}
};
var frCommon = {
...
async function initI18n(wmeSDK) {
...
function validateFeature(raw) {
    ...
    const obj = raw;
    if (obj["type"] !== "Feature") {
        throw new TrackLoadError(`Expected a GeoJSON Feature, got type="${String(obj["type"])}" instead.`);
    }
    ...
    if (geoType !== "LineString" && geoType !== "MultiLineString") {
        throw new TrackLoadError(`GeoJSON has invalid or unsupported geometry type: "${String(geoType)}". ` +
            `Expected "LineString" or "MultiLineString".`);
    }
    const firstCoord = extractFirstCoordinate(geo, geoType);
    if (firstCoord !== null) {
        assertWgs84Coordinate(firstCoord);
    }
    return raw;
}
function extractFirstCoordinate(geo, geoType) {
...
function assertWgs84Coordinate(coord) {
    const lon = coord[0];
    const lat = coord[1];
    ...
    if (!lonInRange || !latInRange) {
        throw new TrackLoadError(`First coordinate [${lon}, ${lat}] is outside WGS84 bounds. ` +
            `coordinates appear to be in a projected CRS (LV95?) instead of WGS84.`);
    }
}

function normalizeTrack(feature) {
    ...
    const track = { trackId, geometry };
    if (rawProperties !== undefined) {
        track.rawProperties = rawProperties;
    }
    return track;
}
function toMultiLineString(geometry) {
...
function extractTrackId(feature) {
    const id = feature["id"];
...
function extractRawProperties(feature) {
...
function fetchJson(url) {
    return new Promise((resolve, reject) => {
        ...
        GM.xmlHttpRequest({
            ...
            onerror(response) {
                reject(new TrackLoadError(`Network error fetching GeoJSON from ${hostname}: ${response.statusText || "unknown error"}.`));
            },
            ontimeout() {
                reject(new TrackLoadError(`Request timed out after ${FETCH_TIMEOUT_MS / 1000}s fetching GeoJSON from ${hostname}.`));
            },
        });
    });
}
...
const logger = {
    info: (msg, ...args) => console.info("[WME-geojson]", msg, ...args),
    warn: (msg, ...args) => console.warn("[WME-geojson]", msg, ...args),
    error: (msg, ...args) => console.error("[WME-geojson]", msg, ...args),
    debug: (msg, ...args) => console.debug("[WME-geojson]", msg, ...args),
};
...
var jsts = /*@__PURE__*/getDefaultExportFromCjs(jsts_minExports);
...
function computeDistanceLabels(geometry) {
    const labels = [];
    let cumulativeKm = 0;
...
function segmentDistanceKm(a, b) {
    return distance([a[0], a[1]], [b[0], b[1]], { units: "kilometers" });
}
function formatLabelKm(km) {
    if (km < 1)
...
class TrackLayer {
    ...
    setVisibleDistances(distancesKm) {
        if (!this.currentTrack)
            return;
    ...
    filterLabels() {
        ...
        return Array.from(this.allLabels.reduce((acc, l) => {
            if (l.km < lo || l.km > hi)
                return acc;
    ...
    drawTrackInRange(lo, hi) {
        if (!this.currentTrack)
            return;
        const baseId = this.currentTrack.trackId !== null
            ? String(this.currentTrack.trackId)
            : `track-${Date.now()}`;
        let cumulativeKm = 0;
        this.currentTrack.geometry.coordinates.forEach((lineCoords, index) => {
            const lineLengthKm = sumSegmentDistances$1(lineCoords);
            ...
            const clippedCoords = sliceLineByDistance$1(lineCoords, localLo, localHi);
            if (clippedCoords.length < 2)
                return;
            this.addLineFeature(`${baseId}-line-${index}`, clippedCoords);
        });
    }
    addLineFeature(featureId, lineCoords) {
        const coords2d = lineCoords.map((c) => [c[0], c[1]]);
    ...
    buildStyleRules() {
        return [
            {
                predicate: (props) => props.kind === LINE_KIND,
                style: {
            ...
            {
                predicate: (props) => props.kind === LABEL_KIND,
                style: {
                    label: "${getLabel}",
...
TrackLayer.LAYER_NAME = "wme-geojson-track";
function labelFeatureId(label) {
    return `label-${label.subLineIndex}-${label.vertexIndex}`;
}
function sumSegmentDistances$1(line) {
    let total = 0;
    for (let i = 1; i < line.length; i++) {
        const a = line[i - 1];
        const b = line[i];
        total += haversineKm$1(a[0], a[1], b[0], b[1]);
    }
    return total;
}
function sliceLineByDistance$1(line, loKm, hiKm) {
    if (line.length < 2 || hiKm <= loKm)
        return [];
    const result = [];
    let cumulative = 0;
    let started = false;
    for (let i = 0; i < line.length - 1; i++) {
        const a = line[i];
        const b = line[i + 1];
        const segLen = haversineKm$1(a[0], a[1], b[0], b[1]);
        ...
        if (!started) {
            ...
            result.push(interpolatePosition$1(a, b, t));
            started = true;
        }
        if (segEnd <= hiKm) {
            result.push(b);
...
function interpolatePosition$1(a, b, t) {
    return [a[0] + (b[0] - a[0]) * t, a[1] + (b[1] - a[1]) * t];
}
function haversineKm$1(lon1, lat1, lon2, lat2) {
    const R = 6371;
...
function computeViewportSizeDeg(extent) {
...
function waitForMapIdle(wmeSDK, opts = {}) {
...
const SEGMENT_ZOOM_LEVEL$1 = 17;
const REFERENCE_POINT = { lon: 8.23, lat: 46.82 };
async function measureViewportAtZ17(wmeSDK) {
    ...
    wmeSDK.Map.setMapCenter({
        ...
        zoomLevel: SEGMENT_ZOOM_LEVEL$1,
    });
    await waitForMapIdle(wmeSDK);
    const extent = wmeSDK.Map.getMapExtent();
    const size = computeViewportSizeDeg(extent);
    logger.info(`measureViewport: lonSpan=${size.lonSpan.toFixed(4)}°, latSpan=${size.latSpan.toFixed(4)}°`);
    return size;
}

function cellBbox(lon, lat, halfLonSpan, halfLatSpan) {
    return [lon - halfLonSpan, lat - halfLatSpan, lon + halfLonSpan, lat + halfLatSpan];
}
function trackFirstPoint(track) {
...
function planWalk(args) {
    ...
    const buffered = buffer(trackFeature, bufferMeters, { units: "meters" });
    if (!buffered) {
        return [];
    }
    ...
    for (let lat = gridMinLat; lat <= gridMaxLat + stepLat * 0.01; lat += stepLat) {
        for (let lon = gridMinLon; lon <= gridMaxLon + stepLon * 0.01; lon += stepLon) {
            ...
            const cellPoly = bboxPolygon(bbox);
    ...
    let current = candidates.reduce((best, cell) => {
        const dBest = distance([best.center.lon, best.center.lat], [startPoint.lon, startPoint.lat], { units: "degrees" });
        const dCell = distance([cell.center.lon, cell.center.lat], [startPoint.lon, startPoint.lat], { units: "degrees" });
    ...
    while (remaining.size > 0) {
        ...
        for (const candidate of remaining) {
            const d = distance([current.center.lon, current.center.lat], [candidate.center.lon, candidate.center.lat], { units: "degrees" });
            if (d < nearestDist) {
                nearestDist = d;
                nearest = candidate;
            }
...
const ALLOWED_TRANSITIONS = new Map([
    ["idle", new Set(["walking"])],
    ["walking", new Set(["done", "cancelled", "error"])],
    ["done", new Set(["idle", "walking"])],
    ["cancelled", new Set(["idle", "walking"])],
    ["error", new Set(["idle", "walking"])],
]);
function isTransitionAllowed(from, to) {
    return ALLOWED_TRANSITIONS.get(from)?.has(to) ?? false;
}

const SEGMENT_ZOOM_LEVEL = 17;
...
class WalkController {
    constructor(wmeSDK, track) {
        ...
        this.matchFoundEmitter = new EventEmitter();
    }
    async start() {
    ...
    async runWalk() {
        ...
        const bufferedTrack = buffer(trackFeature, BUFFER_METERS, { units: "meters" });
        if (!bufferedTrack) {
            logger.error("WalkController: failed to buffer track; aborting walk");
            this.transition("error");
            return;
        }
        const cells = planWalk({
            track: this.track,
            viewportSizeDeg,
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            try {
                ...
                logger.debug(`WalkController: cell ${i + 1}/${totalCells} — ${newIds.length} new segments (${this.matchedIds.size} total)`);
            }
            catch (err) {
                logger.warn(`WalkController: error on cell ${i + 1}/${totalCells}, skipping`, err);
            }
            await new Promise((r) => setTimeout(r, YIELD_BETWEEN_CELLS_MS));
        }
...
function parseDistanceList(input) {
...
function computePortions(distancesKm, totalKm) {
...
function sliceMultiLineByDistance(geometry, kmA, kmB) {
    if (kmA >= kmB) {
...
function bboxOfMultiLineString(geometry) {
    const allCoords = geometry.coordinates.flat();
    if (allCoords.length === 0)
        return null;
    return bbox({ type: "Feature", geometry, properties: null });
}
function sumSegmentDistances(line) {
    let total = 0;
    for (let i = 1; i < line.length; i++) {
        const a = line[i - 1];
        const b = line[i];
...
function sliceLineByDistance(line, loKm, hiKm) {
    if (line.length < 2 || hiKm <= loKm)
        return [];
    const result = [];
    let cumulative = 0;
...
class MatchPanel {
    constructor(wmeSDK, controller, track, trackLayer) {
        this.wmeSDK = wmeSDK;
    ...
    async mount() {
        ...
        this.unsubscribeProgress = this.controller.onProgress((visited, total, _newIds) => {
            if (this.progressEl) {
                this.progressEl.textContent = instance.t("panel.progress.running", {
                    visited,
                    total,
                });
            }
    ...
    buildDOM(container) {
        ...
        title.textContent = instance.t("panel.title");
        container.appendChild(title);
        container.appendChild(this.buildTrackInfo());
        const badgeWrapper = document.createElement("p");
        const badge = document.createElement("strong");
        badge.textContent = instance.t(`panel.status.${this.controller.state}`);
        ...
        progress.textContent = instance.t("panel.progress.empty");
    ...
    buildTrackInfo() {
        ...
        idLine.textContent = instance.t("panel.trackInfo.id", { id: trackIdValue });
        section.appendChild(idLine);
        const lengthLine = document.createElement("p");
        const lengthKm = length({ type: "Feature", geometry: this.track.geometry, properties: null }, { units: "kilometers" }).toFixed(2);
        lengthLine.textContent = instance.t("panel.trackInfo.length", { km: lengthKm });
    ...
    buildRangeSlider() {
        ...
        heading.textContent = instance.t("panel.range.title");
        section.appendChild(heading);
        const valueLabel = document.createElement("p");
        valueLabel.style.margin = "0 0 6px 0";
        valueLabel.style.fontSize = "12px";
        valueLabel.textContent = instance.t("panel.range.window", {
            min: "0.00",
    ...
    buildDistanceFilter() {
        ...
        summary.textContent = instance.t("panel.distanceFilter.title");
        details.appendChild(summary);
        const help = document.createElement("p");
        help.style.margin = "6px 0 4px 0";
        help.style.fontSize = "11px";
        help.style.color = "#555";
        help.textContent = instance.t("panel.distanceFilter.help");
        details.appendChild(help);
        const textarea = document.createElement("textarea");
        textarea.placeholder = instance.t("panel.distanceFilter.placeholder");
        textarea.rows = 3;
        ...
        matchStatus.textContent = instance.t("panel.distanceFilter.matched", {
            visible: 0,
            requested: 0,
        });
        ...
        staleEl.textContent = instance.t("panel.distanceFilter.stale");
        ...
        runBtn.type = "button";
        runBtn.textContent = instance.t("panel.distanceFilter.runProcess");
        btnRow.appendChild(runBtn);
        this.bboxProcessBtn = runBtn;
        const stopBtn = document.createElement("button");
        stopBtn.type = "button";
        stopBtn.textContent = instance.t("panel.distanceFilter.stopProcess");
        ...
        clearBtn.textContent = instance.t("panel.distanceFilter.clear");
        details.appendChild(clearBtn);
        let pendingFrame = 0;
        let pendingDistances = [];
        const updateMatchStatus = (visible, requested) => {
            matchStatus.textContent = instance.t("panel.distanceFilter.matched", {
                visible,
                requested,
            });
        ...
        const applyLabelFilter = () => {
            ...
            if (pendingFrame === 0) {
                pendingFrame = requestAnimationFrame(() => {
                    pendingFrame = 0;
                    this.trackLayer.setVisibleDistances(pendingDistances.length === 0 ? null : pendingDistances);
        ...
        runBtn.addEventListener("click", () => {
            ...
            if (distances.length === 0)
                return;
            const totalKm = this.trackLayer.getTotalKm();
            this.runBboxProcess(distances, totalKm).catch((err) => {
    ...
    async runBboxProcess(distancesKm, totalKm) {
        if (this.bboxProcessRunning)
            return;
        this.bboxProcessAborted = false;
        this.bboxProcessRunning = true;
        if (this.bboxProcessBtn)
            this.bboxProcessBtn.disabled = true;
        if (this.bboxStopBtn)
            this.bboxStopBtn.style.display = "";
        if (this.bboxStatusEl)
            this.bboxStatusEl.style.display = "";
        if (this.bboxStaleEl)
            this.bboxStaleEl.style.display = "none";
        try {
            if (this.bboxListEl) {
            ...
            const allViews = [];
            const bisect = async (inputDistance, kmA, kmB, depth, collector) => {
                if (this.bboxProcessAborted)
                    return;
                ...
                if (zoom >= MIN_BBOX_ZOOM || depth >= MAX_BISECT_DEPTH) {
                    if (depth >= MAX_BISECT_DEPTH && zoom < MIN_BBOX_ZOOM) {
                        logger.warn(`runBboxProcess: depth cap reached for portion [${kmA}, ${kmB}] at z${zoom} — accepting`);
                    }
                    const centerLon = (box[0] + box[2]) / 2;
                    const centerLat = (box[1] + box[3]) / 2;
                    collector.push({ centerLon, centerLat, zoom: zoom });
                    return;
                }
                const mid = (kmA + kmB) / 2;
            ...
            for (const portion of portions) {
                if (this.bboxProcessAborted)
                    break;
                if (this.bboxStatusEl) {
                    this.bboxStatusEl.textContent = instance.t("panel.distanceFilter.processing", {
                        done,
                        total,
                    });
                }
            ...
            if (this.bboxListEl) {
                for (const view of allViews) {
                    ...
                    if (view.totalInGroup === 1) {
                        btn.textContent = instance.t("panel.distanceFilter.viewButton", {
                            distance: distLabel,
                        });
                    }
                    else {
                        btn.textContent = instance.t("panel.distanceFilter.viewButtonIndexed", {
                    ...
                    small.textContent = instance.t("panel.distanceFilter.viewCenter", {
            ...
            this.bboxProcessRunning = false;
            if (this.bboxProcessBtn)
                this.bboxProcessBtn.disabled = false;
            if (this.bboxStopBtn)
                this.bboxStopBtn.style.display = "none";
            if (this.bboxStatusEl)
                this.bboxStatusEl.style.display = "none";
        }
    }
    buildButtons() {
        const wrapper = document.createElement("div");
        const centerBtn = document.createElement("button");
        centerBtn.textContent = instance.t("panel.buttons.centerOnTrack");
        centerBtn.addEventListener("click", () => {
            this.centerOnTrack();
        });
        wrapper.appendChild(centerBtn);
        const startBtn = document.createElement("button");
        startBtn.textContent = instance.t("panel.buttons.start");
        startBtn.addEventListener("click", () => {
            this.controller.start().catch((err) => {
        ...
        const stopBtn = document.createElement("button");
        stopBtn.textContent = instance.t("panel.buttons.stop");
        stopBtn.addEventListener("click", () => {
        ...
        selectAllBtn.textContent = instance.t("panel.buttons.selectAll");
        selectAllBtn.disabled = true;
        selectAllBtn.addEventListener("click", () => {
            this.onSelectAllClick().catch((err) => {
    ...
    buildResultsSection() {
        ...
        countEl.textContent = instance.t("panel.results.count", { count: 0 });
        countEl.style.display = "none";
        section.appendChild(countEl);
        this.resultsCountEl = countEl;
        const emptyEl = document.createElement("p");
        emptyEl.textContent = instance.t("panel.results.empty");
    ...
    appendResultItem(id) {
        ...
        if (this.resultsCountEl) {
            ...
            this.resultsCountEl.textContent = instance.t("panel.results.count", {
        ...
        const li = document.createElement("li");
        const btn = document.createElement("button");
        btn.textContent = instance.t("panel.results.item", { id });
    ...
    onItemClick(id, li) {
        this.controller.focusSegment(id).catch((err) => {
            logger.warn(`MatchPanel: focusSegment failed for id=${id}`, err);
            this.showItemError(li);
        });
    }
    showItemError(li) {
        if (li.querySelector(".wme-geojson-item-error"))
            return;
        const errSpan = document.createElement("span");
        errSpan.className = "wme-geojson-item-error";
        errSpan.textContent = ` ${instance.t("panel.results.unavailable")}`;
    ...
    async onSelectAllClick() {
        ...
        if (needsConfirmation) {
            const confirmed = await confirmModal({
                message: instance.t("panel.modal.largeSelectionWarning", { count }),
                confirmLabel: instance.t("panel.buttons.selectAllConfirm"),
                cancelLabel: instance.t("panel.buttons.cancel"),
            });
            if (!confirmed) {
                return;
            }
    ...
    showSelectionError(errorMessage) {
        if (!this.tabPane)
            return;
        const text = instance.t("panel.errors.selectionFailed", { error: errorMessage });
    ...
    syncActiveItem() {
        ...
        const items = this.resultsList.querySelectorAll("[data-segment-id]");
    ...
    centerOnTrack() {
        const box = bbox(this.track.geometry);
        this.wmeSDK.Map.zoomToExtent({ bbox: box });
    ...
    resetResults() {
        ...
        if (this.progressEl) {
            this.progressEl.textContent = instance.t("panel.progress.empty");
        }
        if (this.selectAllBtn) {
    ...
    updateState(state) {
        if (this.badgeEl) {
            this.badgeEl.textContent = instance.t(`panel.status.${state}`);
        }
    ...
    applyButtonState(state) {
        ...
        const canStart = state === "idle" || state === "done" || state === "cancelled" || state === "error";
...
function getGeojsonUrlFromLocation() {
    ...
    if (!isValidUrl) {
        logger.warn(`"geojson" query parameter is present but not a valid URL: "${decoded}". ` +
